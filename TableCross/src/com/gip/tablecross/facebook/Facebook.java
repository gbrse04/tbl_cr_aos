...
public class Facebook {
	 ...
	 */
	public Facebook(String appId, Activity activity) {
		if (appId == null) {
	...
	private void startDialogAuth(Activity activity, String[] permissions) {
		...
		dialog(activity, LOGIN, params, new DialogListener() {
			...
			public void onComplete(Bundle values) {
				...
				Logger.e(TAG, "authen ok");
				setAccessExpiresIn(values.getString(EXPIRES));
				if (isSessionValid()) {
					mAuthDialogListener.onComplete(values);
					Logger.e(TAG, "session ok");
				} else {
					mAuthDialogListener.onFacebookError(new FacebookError("Failed to receive access token."));
					Logger.e(TAG, "session no ok");
				}
			}

			public void onError(DialogError error) {
				Logger.d("Facebook-authorize", "Login failed: (1) " + error);
				mAuthDialogListener.onError(error);
			}

			public void onFacebookError(FacebookError error) {
				Logger.d("Facebook-authorize", "Login failed: (2) " + error);
				mAuthDialogListener.onFacebookError(error);
			}

			public void onCancel() {
				Logger.d("Facebook-authorize", "Login canceled");
			...
			if (resultCode == Activity.RESULT_OK) {
				...
				if (error != null) {
					if (error.equals(SINGLE_SIGN_ON_DISABLED) || error.equals("AndroidAuthKillSwitchException")) {
						Logger.d("Facebook-authorize", "Hosted auth currently " + "disabled. Retrying dialog auth...");
						startDialogAuth(mAuthActivity, mAuthPermissions);
					} else if (error.equals("access_denied") || error.equals("OAuthAccessDeniedException")) {
						Logger.d("Facebook-authorize", "Login canceled by user.");
						mAuthDialogListener.onCancel();
					} else {
						...
						Logger.d("Facebook-authorize", "Login failed: (3) " + error);
				...
				if (data != null) {
					Logger.d("Facebook-authorize", "Login failed: (4) " + data.getStringExtra("error"));
	...
	public void dialog(Context context, String action, Bundle parameters, final DialogListener listener) {
		...
		if (context.checkCallingOrSelfPermission(Manifest.permission.INTERNET) != PackageManager.PERMISSION_GRANTED) {
			...
			Logger.e("Time process", "" + (end - start));
	...
	// private void putAccessToken(String strFacebookToken) {
	// SharedPreferences pref = mAuthActivity.getSharedPreferences("FACEBOOK",
	// 0);
	...
	class ClearCookie extends AsyncTask<String, String, String> {
		ProgressDialog progressDialog;
		public Context mContext;
		String url;
		DialogListener dialogListener;

		public ClearCookie(Context context, String url, DialogListener dialogListener) {
			mContext = context;
			this.url = url;
		...
		protected void onPostExecute(Long result) {
			...
			new FbDialog(mContext, url, dialogListener).show();
		}
